document.addEventListener("DOMContentLoaded", () => {
  ...
  const navListRef = document.querySelector(".t-c-navigation-list");
  const headerRef = document.querySelector(".t-c-header");
  const navigationRef = document.querySelector(".t-c-navigation");
  ...
  document.querySelectorAll(".t-c-navigation-link").forEach((n) =>
...
function toggleAnswer(element) {
  ...
  const buttonImg = element.querySelector(".t-c-toggle-button img");

  answer.classList.toggle("open");

  if (answer.classList.contains("open")) {
    buttonImg.src = "/img/Plus (2).png";
